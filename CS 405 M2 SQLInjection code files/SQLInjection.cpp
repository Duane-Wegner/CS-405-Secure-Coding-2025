...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
   ...
   * This code detects and blocks SQL injection attempts using patterns like:
   * "OR value=value", where value is a string or number.
   * It works by converting the SQL to lowercase, removing spaces, and matching
   * against a regular expression that looks for the injection pattern.
   */

   // Convert the SQL string to lowercase to make pattern matching case-insensitive
    std::string lowered_sql = sql;
    std::transform(lowered_sql.begin(), lowered_sql.end(), lowered_sql.begin(), ::tolower);

    // Create a copy with all spaces removed to catch variations like "or1=1" or "or'a'='a'"
    std::string condensed_sql = lowered_sql;
    condensed_sql.erase(remove(condensed_sql.begin(), condensed_sql.end(), ' '), condensed_sql.end());

    // Regular expression to detect "or value=value" where value can be a number or quoted string
    std::regex injection_pattern(R"(or('[^']*'|"[^"]*"|\d+)=\1)");

    // If the pattern is found, print a warning and block the query
    if (std::regex_search(condensed_sql, injection_pattern)) {
